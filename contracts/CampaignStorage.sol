...
library CampaignStorage {
  ...
  function setContractOwner(address _newOwner) internal {
    require(
      _newOwner != address(0),
      "Storage::setContractOwner: new owner cannot be the null address"
    );
    StorageStruct storage ds = getStorage();
  ...
  function addFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
    require(
      ...
      "PositionManagerStorage::addFunctions: No selectors in facet to cut"
    );
    StorageStruct storage ds = getStorage();
    require(
      _facetAddress != address(0),
      "PositionManagerStorage::addFunctions: Add facet can't be address(0)"
    );
  ...
  function replaceFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
    ...
    require(
      _facetAddress != address(0),
      "PositionManagerStorage::replaceFunctions: Add facet can't be address(0)"
    );
    uint96 selectorPosition = uint96(
      ds.facetFunctionSelectors[_facetAddress].functionSelectors.length
    );

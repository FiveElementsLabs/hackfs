...
const ConnectButton = () => {
  ...
  const newDid = useMemo(
    () => (connection as any)?.selfID?.id || null,
    [connection]
  );
  ...
  const disconnectCeramic = () => {
    disconnect();
    dispatch({ type: actions.SET_TWITTER_USERNAME, payload: null });
    dispatch({ type: actions.SET_DID, payload: { did: null } });
  };

  return connection.status === "connected" ? (
    <button
      className="inline-flex items-center px-4 py-1 border border-transparent text-sm font-medium 
    ...
    <button
      ...
      className="inline-flex items-center px-4 py-1 border border-transparent text-sm font-medium 
                ...
                focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-indigo-300"
    >
